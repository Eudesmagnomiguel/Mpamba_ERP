...
import { Search, Plus, Trash2, ChevronLeft, Package, 
         ShoppingCart, FileText, Info,
         Minus,Loader2,UserCircle2,Wallet,AlertCircle
} from 'lucide-react';
...
import { CreateProformaSchema, CreateProformaDto } from '@/shared/dto/billing.dto';
import { useBillingCustomers, useBillingSeries, useCreateBillingProforma, useBillingServices } from '@/hooks/module/billing';


export default function NewProformaPage() {
    const router = useRouter();
    const { user } = useAuthStore();
    const { mutate: createProforma, isPending: isCreating } = useCreateBillingProforma();
    ...
    const series = Array.isArray(seriesData) ? seriesData.filter((s: any) => s.prefix === 'PF') : [];
    ...
    const {
        ...
        resolver: zodResolver(CreateProformaSchema),
        defaultValues: {
            seriesId: '',
            customerId: null,
            customerName: '',
    ...
    const onSubmit = (data: CreateProformaDto) => {
        if (!selectedCustomer) {
            toast.error('Por favor, selecione um cliente para a proforma');
            return;
        }
        if (items.length === 0) {
            toast.error('Adicione pelo menos um item à proforma');
            return;
        }
        ...
        createProforma(data, {
            ...
            onError: (err: any) => {
                const msg = err?.response?.data?.message || err?.message || 'Erro ao criar proforma';
                toast.error(msg);
            }
        });
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            <h1 className="text-xl font-black text-slate-900 tracking-tight">Nova Proforma</h1>
                        ...
                        <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Documento de cotação não vinculativo</p>
                    </div>
                </div>
                    ...
                    {series.length > 0 && (
                        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-sm px-3 h-11 shadow-sm">
                            ...
                            <div className="flex flex-col">
                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter -mb-1">Série PF</span>
                                <select
                                    {...register('seriesId')}
            ...
            <div className="flex flex-1 gap-6 overflow-hidden min-h-0">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex items-start gap-6">
                                <div className="flex-1">
                                    <FormSelect 
                                        placeholder="Escolha um cliente para o orçamento..."
                    ...
                    <div className="flex flex-col gap-4 overflow-hidden">
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="relative flex-1 max-w-md">
                                ...
                                <Input
                                    placeholder="Procurar itens para a proforma..."
                                    ...
                                    products.length > 0 ? products.map((p: any) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-end justify-between border-t border-slate-50 pt-3">
                                                ...
                                                <div className="px-2 py-0.5 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-sm text-[9px] font-black uppercase tracking-tighter">
                                                    Em Stock
                                                </div>
                                            </div>
                ...
                <div className="flex-[3] flex flex-col min-w-[320px] max-w-[380px] bg-white border border-slate-200 rounded-sm shadow-xl overflow-hidden animate-in slide-in-from-right-6 duration-500 relative">
                    ...
                    <div className="px-6 py-5 border-b border-slate-100 shrink-0 relative z-10 bg-slate-50/50">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-xs font-black text-slate-900 uppercase tracking-widest leading-none">Orçamento</span>
                                    <span className="text-[10px] text-slate-400 font-bold uppercase mt-1">{items.length} Itens na proforma</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto px-2 py-4 custom-scrollbar relative z-10">
                        {fields.length > 0 ? fields.map((field, index) => {
                            ...
                            <div className="h-full flex flex-col items-center justify-center p-12 text-center">
                                ...
                                <p className="text-sm font-black text-slate-900 uppercase tracking-widest">Orçamento Vazio</p>
                                <p className="text-[11px] text-slate-400 font-bold uppercase mt-2 leading-relaxed">Selecione itens do catálogo para iniciar a cotação</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="px-6 py-6 bg-slate-50/80 border-t border-slate-100 relative z-10">
                        ...
                        <p className="text-[9px] text-center text-slate-300 font-bold uppercase tracking-[0.2em] mt-6">Mpamba Billing System • Quotation Tool</p>
                    </div>
                </div>
            </div>
        </form>
