...
import { useBillingReceipts, useIssueReceipt } from '@/hooks/module/billing';
...
export default function ReceiptsPage() {
    ...
    const { data: receiptsData, isLoading, error } = useBillingReceipts({ search: searchTerm });
    const { mutate: issueReceipt, isPending: isIssuing } = useIssueReceipt();

    const receipts = receiptsData?.data || [];

    const handleIssue = (id: string) => {
        if (!confirm('Deseja emitir este recibo oficialmente?')) return;
        ...
        issueReceipt(id, {
            ...
            onError: (err: any) => {
                const msg = err?.response?.data?.message || 'Erro ao emitir recibo';
                toast.error(msg);
            }
        });
    };
    ...
    const handleDownload = async (id: string, number: string) => {
        try {
            ...
            link.href = url;
            link.setAttribute('download', `recibo-${number || 'rascunho'}.pdf`);
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            default:
                return { label: status, color: 'bg-slate-50 text-slate-600', icon: ReceiptIcon };
        }
    };
    ...
    return (
        <div className="space-y-6 pb-12 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Recibos e Quitações</h1>
                    <p className="text-slate-500 text-sm font-medium mt-1">Comprovativos de pagamento e quitações emitidas.</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <ExportExcelButton
                        ...
                        fetchFile={() => billingExportService.exportReceipts({ search: searchTerm || undefined })}
                    />
                    <button 
            ...
            <div className="bg-white border border-slate-200 rounded-sm overflow-hidden shadow-sm min-h-100 flex flex-col">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                ...
                                <th className="px-6 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Fatura Referente</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Valor Pago</th>
                            ...
                            {isLoading ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-20 text-center">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Carregando recibos...</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : error ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-20 text-center text-red-500 font-bold uppercase tracking-widest text-xs">
                                        Erro ao carregar histórico de recibos.
                                    </td>
                                </tr>
                            ) : receipts.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-20 text-center">
                                        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Nenhum recibo encontrado.</p>
                                    </td>
                                </tr>
                            ) : (
                                receipts.map((rc: any) => {
                                    const badge = getStatusBadge(rc.status);
                                    ...
                                    return (
                                        <tr key={rc.id} className="hover:bg-slate-50/50 transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className={cn(
                                                        ...
                                                        rc.status === 'ISSUED' ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-slate-50 text-slate-500 border-slate-100"
                                                    )}>
                                                        RC
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-bold text-slate-900 leading-none">{rc.number || 'RASCUNHO'}</p>
                                                        <p className="text-xs font-bold text-slate-400 mt-1.5 uppercase tracking-tighter">
                                                            {new Date(rc.date).toLocaleDateString('pt-AO', { day: '2-digit', month: 'short', year: 'numeric' })}
                                                        </p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div>
                                                    <p className="text-sm font-semibold text-slate-700 leading-none">{rc.invoice?.number || 'Fatura #'}</p>
                                                    <p className="text-[10px] font-bold text-slate-400 mt-1.5 uppercase">{rc.paymentMethod || 'N/A'} - {rc.reference || 'Sem Ref'}</p>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="text-sm font-bold text-emerald-600">
                                                    {rc.amount.toLocaleString('pt-AO', { style: 'currency', currency: 'AOA' })}
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    {rc.status === 'DRAFT' && (
                                                        <button 
                                                            onClick={() => handleIssue(rc.id)}
                                                            disabled={isIssuing}
                                                            className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-sm transition-all"
                                                            title="Emitir Recibo"
                                                        >
                                                            {isIssuing ? <Loader2 size={18} className="animate-spin" /> : <CheckCircle2 size={18} />}
                                                        </button>
                                                    )}
                                                    <button 
                                                        onClick={() => handlePrint(rc.id)}
                                                    ...
                                                    <button 
                                                        onClick={() => handleDownload(rc.id, rc.number)}
                ...
                <div className="px-6 py-4 bg-slate-50/30 border-t border-slate-100 flex items-center justify-between mt-auto">
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
                        Total: {receiptsData?.meta?.total || receipts.length} documentos
                    </p>
